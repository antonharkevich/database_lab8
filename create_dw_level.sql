...
create table TS_DW_DATA_USER.DW_GEO_LOCATIONS 
(
   ...
   insert_dt            date                           not null,
   update_dt            date                           not null,
   constraint PK_DW_GEO_LOCATIONS primary key (territory_id)
)
tablespace ts_dw_data;

create table TS_DW_DATA_USER.DW_GAMES_SCD 
(
   ...
   insert_dt            date                           not null,
   constraint PK_DW_GAMES_SCD primary key (game_surr_id)
)
tablespace ts_dw_data;
...
create table TS_DW_DATA_USER.DW_CUSTOMERS 
(
...
create table TS_DW_DATA_USER.DW_COMPANIES 
(
   ...
   insert_dt            date                           not null,
   update_dt            date                           not null,
   constraint PK_DW_COMPANIES primary key (company_id)
)
tablespace ts_dw_data;
...
alter table TS_DW_DATA_USER.DW_SALES
   add constraint FK_FCT_SALE_REFERENCE_DW_GEO_LOCATIONS foreign key (territory_id)
      references TS_DW_DATA_USER.DW_GEO_LOCATIONS (territory_id)
      on delete cascade;

alter table TS_DW_DATA_USER.DW_SALES
   add constraint FK_FCT_SALE_REFERENCE_DW_GAME foreign key (game_surr_id)
      references TS_DW_DATA_USER.DW_GAMES_SCD (game_surr_id)
     on delete cascade;

alter table TS_DW_DATA_USER.DW_SALES
   add constraint FK_FCT_SALE_REFERENCE_DW_COMPANIES foreign key (company_id)
      references TS_DW_DATA_USER.DW_COMPANIES (company_id)
      on delete cascade;

alter table TS_DW_DATA_USER.DW_SALES
   add constraint FK_FCT_SALE_REFERENCE_DW_CUSTOMERS foreign key (customer_id)
      references TS_DW_DATA_USER.DW_CUSTOMERS (customer_id)
      on delete cascade;
...
alter table TS_DW_DATA_USER.DW_SALES
   add constraint FK_FCT_SALE_REFERENCE_DW_GEN_PERIODS foreign key (sales_cat_id)
      references TS_DW_DATA_USER.DW_GEN_PERIODS  (sales_cat_id)
      on delete cascade;